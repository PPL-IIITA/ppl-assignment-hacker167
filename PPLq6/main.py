...
import pprint
from q6_utility import utility
...
def hap_cpl(L, k,B,G):
                ...
                if (t==0):
                    t=t+1
                k=randint(100,1000000)+t
                A = sorted(L, key=lambda x: x.c_hap)
                """sort list L in ascending order according to happiness"""
                newcpl(L,k,B,G,A)
                
def newcpl(L,k,B,G,A):
	'allocates new boys to the girls who broke up'
   	R = []
   	k1=len(L)
	for i in range(k1):
		for j in L:
			if (A[i].g.name_id == j.g.name_id and L.c_hap<k):
				j.b.status = 'single'
				j.b.gf = ''
				j.b.happiness = 0
				j.g.status = 'single'
	...
	for r in R:
		L.remove(r)
	logging.warning('new search for bf:\n')
	for r in R:
		for b in B:
			logging.info('Commitment:  Girl: ' + r.g.name_id + '  is checking out  Boy: ' + b.name_id)
                        if r.g.status=='single' and b.status=='single' and r.g.constraint(b.bgt) and b.constraint(r.g.main_bgt, r.g.attr)and b.name_id!=r.b.name_id :
                                i.status= 'Committed'
                                j.status = 'Committed'	
                                #display(i,j)
                                j.gf=i.name_id;
...
def mk_cpl():
                ...
                for row in bp:
                    if(row[5]=='Miser'):
                       ...
                       B.append(row[0], int(row[1]), int(row[2]), int(row[3]), int(row[4]), row[5])
                ...
                for row in bp:
                    if(row[4]=='Choosy'):
                       ...
                       G.append(row[0], int(row[1]), int(row[2]), int(row[3]),row[4])
