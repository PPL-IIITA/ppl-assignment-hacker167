...
import pprint
from q10_utility import utility
...
def randomfun(alist,criteria):
        k=randint(1,10)
	if(criteria=='bgt'):
		A=sorted(alist,key=lambda item: item.bgt,reverse=True)
	else:
		A=sorted(alist,key=lambda item: item.val,reverse=True)
	'finding and returning random k valued items depending upon criteria'
	B=random.sample(A,k)
        return B
...
def hap_cpl(L):
		...
		k=randint(1,len(L))
                ...
                A = sorted(L, key=lambda x: x.c_hap,reverse=True)
                print ('\ Happy couples :')
                for i in range(k):
                        print (A[i].b.name_id + ' <-> ' + A[i].g.name_id)
                """sort list L in descending order according to happiness to find k most happiest  couples"""
...
'function call to create and write csv file'
utility()
'function call for making couple'
mk_cpl()
